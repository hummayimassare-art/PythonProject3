...
def convert_timestamp(value):
    ...
    if pd.isna(value):
        ...
        return value.strftime('%Y-%m-%d %H:%M:%S')
    elif isinstance(value, date):  # <-- NEW: handle date objects
        ...
        try:
            ...
            if 'upload_date' in df.columns:
                df['upload_date'] = pd.to_datetime(
                    df['upload_date'],
                    format='mixed',  # lets pandas infer per value
                    errors='coerce'  # invalid values become NaT instead of crashing
                )
...
def load_csv_data():
    ...
    try:
        ...
        try:
            cyber_df = pd.read_csv("cyber_incidents.csv")
...
# ========== FOOTER ==========
st.markdown("---")
st.caption("Multi-Domain Intelligence Platform By HIM")